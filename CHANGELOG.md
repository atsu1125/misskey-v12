...
- アンテナのタイムラインから個別のノートを削除できるように @samunohito @atsu1125
...
- RSS取得用APIのバリデーション強化と同時処理数・レスポンスサイズの制限 @atsu1125
...
- 不可視ノートへのリアクションのエラーハンドリングを追加 @atsu1125
- ドライブファイル削除でデットロックとなりアカウント削除が行われないケースを修正 @zyoshoka @atsu1125
...
- ２段階認証のバックアップコードのダウンロードUIを修正 @atsu1125
...
- 連合への配送でインスタンスごとにサーキットブレーカーを付けてカスケード障害を防止するように @atsu1125
- ワードミュートをリノート対象とリプライ対象の投稿についても適用する @atsu1125
- Node 18からNode 20にアップデート @atsu1125
- misskey-dev/summalyをアップデート @atsu1125
- Node 20, 22, 24のCIテストを追加 @atsu1125

### Bugfixes
- 見つけるのアンケートは非ログイン状態ではAPIコールできないので非表示に @atsu1125
- チャートのツールチップが画面に残ることがあるのを修正 @syuilo
- use-tooltip の呼び出し元の UI が無くなったら自動的に削除されるようにする @YAVIIGI
- インスタンスミュートのクエリを削減 @atsu1125
- リモートユーザーに通知を作成するのを抑制 @zyoshoka @kakkokari-gtyih @syuilo
- メールドメインブロックで正しくドメインが認識されないことがあるのを修正 @atsu1125
- 公開以外へのリプライ時は元の公開範囲を引き継ぐ際にデフォルト公開範囲より上がらないように @atsu1125
- ローカルのみ投稿はグローバルタイムラインに表示しないように @atsu1125
- 「みつける」のリモートユーザーの項目はローカルユーザーは除外するように @atsu1125
- 公開範囲がダイレクトの投稿にはダイレクトでのみリプライ可能にする @atsu1125
- 凍結した際にユーザーのtoot:suspendedをリモートに正しく配信できるようにする @atsu1125
- 凍結しているリモートユーザーについてUpdate, Undo, Reject, and Deleteのアクティビティは許可するように @atsu1125
...
- 自分とモデレーター以外のユーザーから二要素認証関連のデータが取得できないように @kakkokari-gtyih @syuilo @atsu1125
